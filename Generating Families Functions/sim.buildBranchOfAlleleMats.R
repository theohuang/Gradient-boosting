...
#' indicating which alleles the person has for each of the \code{length(q)} 
#' genes, simulated based on the supplied allele frequencies.
#' @param q allele frequencies for each gene of interest 
#' @family simulations
sim.simAlleleMat = function(q) {
  return(sapply(q, function(x){replicate(2, rbinom(1, 1, x))}))
}
...
#' @family simulations
sim.buildBranchOfAlleleMats = function(q, nProg, alleles, allelesSpouse) {
  ...
  if (isAddHead) {
    alleles = sim.simAlleleMat(q)
  }
  # Simulate the spouse's allele matrix if not passed in 
  if (isAddSpouse) {
    allelesSpouse = sim.simAlleleMat(q)
  }
