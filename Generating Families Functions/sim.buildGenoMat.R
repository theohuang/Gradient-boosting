...
#' @param q allele frequencies for each gene of interest
...
sim.buildGenoMat = function(q, nChildPatern, nChildMatern, 
  ...
  allelesGrandchildInBranches = abind(lapply(1:nrow(nGrandchildInBranches), function(i){
    sim.buildBranchOfAlleleMats(q, sum(nGrandchildInBranches[i,]), matrix(allelesChild[i,,], nrow=2))
  }), along=1)
  ...
  genoMat = matrix(t(apply(alleles, 1, sim.allele2Geno)), ncol=length(q))
  if (is.null(names(q))) {
    colnames(genoMat) = letters[1:ncol(genoMat)]
  } else {
    colnames(genoMat) = names(q)
  }
  return(genoMat)
}