...
#' @family simulations
sim.simCancerVars = function(genoMat, Gender, CurAge, CP, age.max = 110,
  ...
  if(censoring == TRUE){
    ...
    # Centered at 85, but values above age.max are re-simulated to be centered around age.max - 15. 
    deathAge = round(rnorm(N, 85, 10))
    while (any(deathAge > age.max)) {
      deathAge[deathAge > age.max] = round(rnorm(sum(deathAge > age.max), age.max - 15, 5))
    }
  ...
  colnames(isAffectMat) = paste0("isAff", cancers)
  # Set up matrix of Age variables, one for each cancer
  if(affTime == TRUE){
    ageMat = matrix(nrow=N, ncol=length(cancers)*2)
    colnames(ageMat) = c(paste0("Age", cancers), paste0("AffAge", cancers))
  } else{
    ageMat = matrix(nrow=N, ncol=length(cancers))
    colnames(ageMat) = paste0("Age", cancers)
  }
  ...
  for (j in 1:length(cancers)) { # Iterate through cancers
    ...
    for(i in 1:N){ # Iterate over each person
      ...
      affectedTime[i] = which(rmultinom(1, 1, prob=cancerDens[[Gender[i]+1]][,genoCombsIdx[i],j])[,1]==1)
    }
    
    # If the person was affected at or before current age, the person is considered 
    # affected for that cancer
    isAffectMat[,j] = ifelse((affectedTime <= CurAge & affectedTime != age.max + 1), 1, 0)
